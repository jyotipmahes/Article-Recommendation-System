...
def load_articles(articles_dirname, gloves):
    ...
    for f in flist:
        text=get_text(f)
        filename=os.path.basename(os.path.dirname(f))+'/'+os.path.basename(f)
        title=text.split('\n')[0]
        rest_text=" ".join(text.split('\n')[1:])
        centroid=doc2vec(rest_text,gloves)
        record_list.append([filename,title,rest_text,centroid])
...
def doc2vec(text, gloves):
    ...
    """
    word=words(text)
    word_in_glove=[w for w in word if w in gloves.keys()]

    vector_matrix=np.array([gloves[w] for w in word_in_glove])
    centroid=np.mean(vector_matrix,0)
...
def distances(article, articles):
    ...
    distance=[]
    for a in articles:
        d=np.linalg.norm(article[3]-a[3])
        distance.append((d,a))
...
def recommended(article, articles, n):
    ...
    """
    different_article=[w for w in articles if w!=article]
    dist=distances(article,different_article)

    top_list=sorted(dist, key=lambda x: x[0])[:n]
    article_list=[a[1] for a in top_list]
